...
def pattern_analysis():
    ...
    dicts = {
        "對門相沖":"「對門相沖」常見於現代住宅大樓內，像是大門對大門、大門對陽台門、臥室門對臥室門、臥室門對廁所門、臥室門對廚房門等等，在風水上而言，可能會產生感情不睦、漏財、身體健康受影響等問題。化解方法就是各自在門上掛門帘或者珠簾，阻擋氣場往來。",
        ...
        "浴廁居中":"「浴廁居中」住宅的中心點為家運旺衰的主要關鍵位置，可以為客廳、書房、臥室但絕對不可以為廁所，在風水上而言，可能會有身體健康受影響、家運受壓無法發展、讀書不利、考運不吉、財運不佳的問題。化解方法：在廁所內擺上黃金葛、投射燈，來淨化穢氣活化氣場，並在門口掛上長布簾來阻絕穢氣外泄。"
        }
...
def predict_analysis():
    ...
    predict_result = price_mod.price_predict(
        city, region, bedroom, liv, bath, house_age,gar, floor, floor_all, area, house_type, house, material, org)
    # 建立session
    SqlalchemySession = sessionmaker(bind=db.engine)
    db_session = SqlalchemySession()
    # 將預測結果存入資料庫
    predict = Predict(name, city, region, bedroom, liv, bath, house_age,gar,
